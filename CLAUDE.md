...
**임계값·상한 상수는 대역에 경계 양쪽을 둔다.** 한쪽에만 있으면 임계값을 바꿔도 결과가
같아 변이가 통과한다 — `MIN_SNIPPET_CHARS` 6→4 가 픽스처 `a=b`(3자) 때문에 안 잡혔고,
`a = b`(5자, 버림) + `a == b`(6자, 남김) 으로 양쪽을 고정한 뒤에야 깨졌다.

전례는 `TROUBLESHOOTING.md` 의 '테스트 · 검증 · 도구' 절에 있다(줄 번호는 행이 늘면 밀린다).
...
**`CLAUDE.md` 를 고치면 `docs/log/` 에 남긴다.** 무엇을 왜 넣었는지 기록이 없으면
다음 세션이 그 절을 누가 왜 썼는지 가릴 수 없다 — 실제로 §8 을 넣고 로그를 안 남겨서
다음 세션이 출처를 확인하지 못했다. 행동 규칙은 이후 모든 작업에 걸리므로 근거가 더 중요하다.
...
코드에서 저장소 이름을 적어도 되는 곳은 **`tests/`(평가셋·픽스처)뿐이다.**
평가셋을 늘릴 때도 `tests/search_eval_dataset.py` 의 `EVAL_SETS` 에만 추가한다.

**문서는 §7 대상이 아니다** — `README.md`·`docs/log/`·`STATUS.md`·`plan.md` 는 코드가 아니라
결과 보고서다. §7 이 막는 것은 프로덕션 코드가 특정 저장소를 알고 거기 맞춰 튜닝하는 것인데,
보고서에는 그 경로가 없다. **이름을 빼면 재현이 불가능해지고, 측정을 믿게 만드는 구체성도
사라진다** — 재현 가능성이 익명성보다 중요하다. 평가셋에 이미 공개돼 있기도 하다.

## 8. 글쓰기 규칙 (모든 프로젝트 공통)

무엇을 쓰든(문서·MR·코드 주석·답변) 가독성을 최우선으로 한다. 쓰는 언어에 따라 규칙이 다르다.

### 한국어

- 한국 개발자가 실제로 많이 쓰고 바로 알아듣는 쉬운 단어로 쓴다.
- 압축 한자어를 명사로 쌓지 않는다("실증", "정합(성)", "무결점", "발동", "절단", "관통", "판정", "외삽" 같은 단어). 동사가 있는 자연스러운 문장으로 쓴다.
  - 예: "종료 유예 실증" → "배포 중에도 진행 중인 답변이 끝까지 전달되는 것을 확인했다"
  - 예: "과금 정합 361=361" → "과금 기록이 361건 = 361건으로 맞았다"
- 화살표 연쇄(A→B→실패)와 괄호 중첩, 어색한 비유("객체 고아", "죽음", "방어선")를 피한다.
- 도메인 용어(파드, 원장 등)는 첫 등장 시 짧은 풀이를 붙인다.
- 영어 약어는 첫 등장 시 풀어 쓴다(e2e → end to end).
- 기본 독자는 뭐든지 공부하려는 AI 엔지니어, 백엔드(자바) 엔지니어, 프롬프트 엔지니어 개발자로 상정한다.

### 영어

- 한국어를 그대로 옮기지(직역하지) 않는다. 영어권 개발자가 자연스럽게 읽도록 처음부터 영어로 다시 쓴다.
- 그쪽에서 흔히 쓰는 개발 표현(fail-open, stale, backpressure, single-flight, cold start 등)을 그대로 쓰고, 직역해서 어색한 영어(콩글리시)가 되지 않게 한다.
- 같은 내용을 두 언어로 낼 때도 영어는 한국어 문장의 번역이 아니라 영어 나름의 자연스러운 문장이어야 한다.
