...
| 인용 추출 · 유실 | **535 → 807** · **358 → 86** | 저장된 답변 8기록에 옛 파서와 새 파서를 각각 돌려 건별로 대조. LLM 호출 0회 |
| 경로 해석 실패 | **171 → 0** | 위와 같은 대조. 새로 만든 잘못된 링크 0건, 전에 맞던 것이 틀리게 된 건 0건 |
| 행 번호 정확도 | **70.7% (162/229)** | 인용이 가리킨 행과 보관 원문을 대조. **서버는 이걸 고쳐 주지 않는다** — 틀린 것을 감추면 고칠 수가 없다 |
| 테스트 | **499건** (무과금) | `pytest -q` 의 기본 실행 대상. 과금·측정 14건은 `-m evaluation`·`-m billed` 로만 돈다. DB 통합 테스트는 Postgres 가 있어야 돌고 없으면 skip 한다 |
...
[10-citation-paths.md](docs/log/10-citation-paths.md).
