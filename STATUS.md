...
- **하네스의 절대 비용을 예산 값으로 쓰지 말 것 — 접두사가 14배 다르다.**
  하네스는 `context` 를 한 줄로 두어 캐시 접두사가 **약 1,140토큰**인데, 프로덕션은
  README·매니페스트·파일 목록이 다 들어가 **16,240토큰**이다(실측). 그래서 질문당
  ...
  **판정(기준 C)은 흔들리지 않는다** — 같은 실행 안의 rag 대비 배수이고 두 팔이 같은
  ...
  프로덕션 기준선은 `runs` 의 실사용 기록(`round_trips` 포함)으로 뽑을 것.
