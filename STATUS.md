...
- **`onMiss` 감시 경로는 아직 아무도 동작을 못 봤다.** 인라인 링크를 못 건 인용을 세는
  경로인데(`Front/src/citations.ts`), 실데이터에 marker 를 못 찾는 사례가 없어 실측이
  줄곧 0건이다. 링크가 다 걸린다는 뜻이라 좋은 결과지만 **감시 자체가 도는지는 미검증**이다.
  대역으로 확인하려면 **marker 가 어긋나는 상황을 만들어야 하는데**, 백엔드가 준 offset·marker
  가 원문과 일치하는 한 그런 상태가 자연히 안 생긴다 — 렌더러가 글자를 바꾸는 경우(마크다운
  강조가 marker 를 가르는 등)를 찾아내거나 억지로 빚어야 한다. **그 대역 설계가 별도 과제다.**
...
- **방금 받은 답변에는 인용 링크가 안 뜬다 — 한 줄짜리 결함.** `Front/src/Chat.tsx:212` 가
  `const { answer } = await res.json()` 로 **`citations` 를 안 꺼낸다.** 그래서 라이브
  경로의 assistant 메시지에는 인용이 없고, 인라인 링크도 '근거' 목록도 안 그려진다.
  `POST /chat` 응답에는 들어 있고(`ChatResponse`), 이력 복원(`App.tsx:78`)은 싣는다 —
  **새로고침하면 살아난다.** 운영 스택에서 실제 질문 하나로 확인했다(서버 인용 7건, 화면 0건).
  **`onMiss` 가 줄곧 0건이던 이유가 이것이다** — 라이브 경로에 인용이 0건이라 놓칠 것이 없다.
  위의 `onMiss` 과제는 이것을 고친 뒤에야 의미가 생긴다.
