...
- **하네스의 절대 비용은 프로덕션보다 낮다 — 예산으로 쓰지 말 것.** 하네스는 `context` 를
  한 줄로 두어 캐시 접두사가 약 1,140토큰인데 프로덕션은 README·매니페스트·파일 목록이
  다 들어가 **16,240토큰**이다. 실측 질문당 비용이 하네스 $0.0161 vs 프로덕션 $0.0225 로
  갈렸다. **판정은 흔들리지 않는다**(기준 C 는 같은 실행 안의 배수이고 두 팔이 같은
  접두사를 썼다). 프로덕션 기준선이 필요하면 `runs` 의 실사용 기록으로 다시 뽑을 것.
- **CRLF 저장소에서 `grep` 을 실환경 확인하지 못했다.** 보관된 228개 파일 중 CRLF 가
  0개다. 단위 테스트로만 덮여 있다.
- **marryday 재색인이 남았다** — 4,365청크, 29~75분. 큐가 하나라 그동안 다른 색인이
  대기하고 임베딩이 CPU 를 다 쓴다. 트래픽이 적은 시간대에 사람이 시작한다.
