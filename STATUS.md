...
- **행 표기를 잡고도 버린 건수를 센다** — `extract()`·`for_answer()` 의 `dropped` Counter,
  ...
  유실은 못 본다. 현재 남은 유실은 파일명 없음 78 · 접미사 중복 7 · 범위가 너무 넓음 1.
  **경로 해석 실패는 0이다** — 171건 전부가 파일이 아닌 문자열이었고 위 규칙이 걷어냈다.
  **파일 밖 범위는 유실이 아니다** — 그건 화면이 보여 줘야 하는 것이다.
- **서버는 인용이 맞았는지 판정하지 않는다.** 행 번호 실측 정확도가 **70.7%**(162/229)인데
...
- **프롬프트가 인용 형식을 지정하지 않는다. 그리고 유일한 예시가 실패 형태다.**
  `CHAT_SYSTEM_PROMPT` 의 `:189` 는 "어느 파일 몇 행인지 밝히세요"라고만 하고 **둘을 같은
  자리에 적으라는 지시가 없다.** 파서의 "직전에 언급된 파일을 잇는다"가 존재하는 이유가
  이것이다. 게다가 형식 예시가 하나뿐인데(`:195` 의 `"42-58행의 \`login()\`"`)
  **파일명이 없는 모양이다** — 프롬프트가 실패 형태를 직접 시범 보이고 있다.
  남은 유실 78건(파일명 없음)이 이 형태이고, 파서로는 못 고친다.
  **고치려면 인용 기준선 재측정이 따라온다**($0.89 ~ $2.13, 07 로그 참고).
  **도입가 만료를 이유로 당기지 않는다** — 값어치가 아니라 달력이 결정 근거가 되는 것은
  이 문서가 상수를 다룰 때 경계해 온 바로 그 패턴이다. B·C 수정으로 유실이 358 → 86 이
  됐으므로, **남은 78건의 값어치를 그 자리에서 다시 계산한 뒤에 정한다.**
