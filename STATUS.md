...
**이름은 '청킹'이지만 청크 경계만 세는 것이 아니다.** `EMBEDDING_MODEL`·
`EMBEDDING_PASSAGE_PREFIX`(§1.1)와 `EMBED_BATCH_SIZE`(§1.3)도 함께 들어간다 —
셋 다 경계를 안 바꾸고 **벡터를 바꾸는데**, 재색인이 필요한 것은 그쪽이기 때문이다.
앞의 둘은 `.env` 에서 오므로 **설정이 다른 환경은 해시도 다르다.**

**설정이 같으면 환경이 달라도 같아야 한다. 한동안 그러지 못했다** — 정규화에 쓰던
`ast.dump()` 가 파이썬 3.13 에서 출력 형식이 바뀌어, 같은 코드·같은 설정인데 로컬
(3.14)과 컨테이너(3.12)가 다른 값을 냈다. `ast.unparse()` 로 바꿔 고쳤고,
`tests/test_chunk_rule.py` 의 `test_normalized_output_is_pinned` 이 그 렌더링을
리터럴로 붙잡는다 — **판이 또 바뀌면 색인이 조용히 낡는 대신 그 테스트가 깨진다**
(2026-08-29, `docs/log/12-index-time.md`).
...
| `NOT_IN_A_PATH` | `18개` | `app.services.citations` | — |
...
- **남용 방지는 세 층이다.** 일일 상한(`DAILY_*`)은 **서비스 전체 합산으로 남는다** —
  ...
  **하나 더** 붙는다. **보는 순서는 IP · 서비스 · 사용자이고, 그 순서가 곧 동작이다**
  (`Back/app/db/rate_limits.py` 의 `reserve`) — IP 에 막힌 요청은 일일 카운터를 올리지
  않고, 사용자 층을 서비스보다 먼저 보면 서비스가 이미 천장에 닿았는데 사용자 카운터만
  올라간다. **익명 요청의 동작은 로그인 도입 전과 같다** — 사용자 층을 아예
...
- **로그인은 배포에서 꺼져 있다. 꺼짐이 기본이다.**
  ...
  **허용 목록도 도입 전과 똑같이 모두에게 걸린다** — 남은 쿠키를 심어도 그렇다는 것을
  `Back/tests/test_analyze_api.py` 가 고정한다.
...
- **`/admin` 에는 인증이 없다. 네트워크 경계가 인증을 대신한다.** `Back/app/api/admin.py`
  ...
  남용 제한도 걸지 않는다**(`Back/app/api/admin.py` 의 `_summarize`). 그래서 **공개 경로에 노출하는 순간 누구나
  ...
  **되돌리기 쉬운 형태다**(정규식에 `admin` 한 단어). 다시 넣는 것은 인증을 붙인
  뒤에 할 일이다(§4 '관리자 인증').
- **배포는 `--force-recreate` 까지 붙여야 반영된다.** `--build` 는 이미지를 새로 굽지만
  컨테이너를 반드시 재생성하지는 않는다. 이미지가 바뀌었는데 옛 컨테이너가 계속 돌면
  **빌드 로그도 healthcheck 도 정상이라 알아챌 신호가 없다** — 고친 파일을 봐도 맞고
  스택도 healthy 다. 5단계 보안 수정에서 실제로 그랬고, 그때는 admin 경로가 그대로
  열려 있었다. 명령은 `docker-compose.prod.yml` 머리말에 있고, 확인은
  `docker inspect <컨테이너> --format '{{.Image}}'` 와 방금 구운 이미지 ID 대조다.
- **허용 목록은 기본이 꺼짐이다.** `ALLOWED_REPOS` 가 비면 임의 저장소를 받는다
  ...
  **목록이 켜져 있어도 로그인한 요청에는 안 걸린다**(`allowlist.check` 의 `user_id`).
  로그인이 꺼진 배포에서는 `user_id` 가 언제나 None 이라 도입 전과 완전히 같고,
  차단 문구도 그때만 옛 문구 그대로다 — 켜져 있을 때만 로그인을 안내한다.
  **목록의 역할이 좁아졌다** — 크기는 이제 `MAX_INDEX_CHUNKS` 가 막고(§2.2), 이 목록은
  로그인하지 않은 사람의 범위를 정한다. 다만 **대기 줄은 아무도 안 막는다**(§4).
- **상시 운영이 아니다. EC2(서울, ap-northeast-2)에서 필요할 때만 켠다.**
  끄고 켤 때마다 **퍼블릭 IP 가 바뀌므로 `.env.prod` 의 `PUBLIC_ORIGIN` 을 같이 고쳐야
  한다** — 안 고치면 `FRONTEND_ORIGIN` 이 옛 출처만 허용한 채 스택은 healthy 로 뜬다.
  그리고 **`down -v` 는 `pgdata` 를 지운다** — 대화 이력과 미리 만들어 둔 색인이 거기 있고,
  지우면 데모 저장소를 처음부터 다시 색인해야 한다(인스턴스 중지·시작으로는 안 지워진다).
  **`.env.prod` 는 커밋되지 않으므로 인스턴스 위의 그 파일이 유일본이다.**
  절차는 `docs/log/09-deploy.md` 의 '재기동 절차'.
...
- **추출은 행 표기가 있으면 다 잡는다. 채점 관문은 채점 쪽에 있다.**
  ...
  추출 관문이라 **화면 경로까지 막아 450건이 버려졌다.** 옮길 당시 정확도는
  72.4%(118/163)·판정불가 102건 그대로였다. `snippets` 는 채점 재료이고 비어도 인용은 만든다.
- **파일명은 보관 목록을 보고 고른다** — `_file_for_line()`. 백틱 안에서 **실제 보관
  경로로 풀리는 것**을 먼저 보고, 없으면 백틱 **밖 평문**에서 풀리는 것을 보고, 그래도
  없으면 백틱 안 마지막 후보를 쓴다(그러면 "경로 해석 실패"로 세어진다 — 직전 줄의
  파일로 이으면 엉뚱한 곳에 링크가 걸린다). `looks_like_path()` 는 **`EXTENSION_TO_LANGUAGE`
  에 있는 확장자로 끝나야** 통과한다 — 보관되는 파일은 그 확장자뿐이므로 **조여도 링크가
  줄지 않는다**(실측 0건). `extract()` 에 목록을 안 주면 옛 규칙(백틱 안 마지막)이다.
- **행 표기를 잡고도 버린 건수를 센다** — `extract()`·`for_answer()` 의 `dropped` Counter,
  `/chat` 이 한 줄로 남긴다. 화면의 `onMiss` 는 **인용을 받은 뒤**를 감시하므로 이쪽
  유실은 못 본다. 현재 남은 유실은 파일명 없음 78 · 범위가 너무 넓음 1(인용 814건).
  **경로 해석 실패는 0이다** — 171건 전부가 파일이 아닌 문자열이었고 위 규칙이 걷어냈다.
  **접미사 중복도 0이다** — 남아 있던 7건은 답변의 잘못이 아니라 파서가 접미사를
  **경계에서 안 끊은** 것이었다(`ApnsPayload.java` 가 `Payload.java` 를 삼켰다).
  **파일 밖 범위는 유실이 아니다** — 그건 화면이 보여 줘야 하는 것이다.
- **경로 매칭은 경계에서 끊는다** — `_matching_paths()` 는 `p == name` 이거나
  `p.endswith("/" + name)` 일 때만 맞다고 본다. 맨 문자열 접미사로 맞추면 파일 이름의
  **중간에서 잘린** 문자열이 맞아, 답변이 파일을 정확히 적어도 둘이 맞아 버려진다.
  **줄여 쓴 경로는 그대로 받는다** — 줄임은 언제나 경계에서 시작한다.
- **한 줄에 파일이 여럿이면 하나만 고른다. 이건 알고 있는 한계다.**
  `_file_for_line()` 은 줄당 파일 하나를 정하므로, "A.java 46행, B.java 55행,
  C.java 75행" 처럼 한 줄에 파일과 범위가 짝지어 여럿 오면 **어느 것을 골라도
  나머지가 틀린다**(실측 답변 2건 × 마커 3개). 마커별로 가장 가까운 파일명을 고르는
  규칙이 있을 수 있지만 안 했다 — §4 참고.
- **서버는 인용이 맞았는지 판정하지 않는다.** 행 번호 실측 정확도가 **71.1%**(165/232)인데
...
- **도구를 아예 안 붙이는 경우는 둘뿐이다** — 전체 주입 스냅샷(소스가 이미 접두사에 다
  들어가 있다)과 **쓸 수 있는 색인이 없는 스냅샷**(`is_ready()` 가 False — 아직 안
  끝났거나, 실패했거나, 청크 상한을 넘어 `skipped` 라 앞으로도 안 만들어진다).
  **한 대화 안에서 안 바뀌는 것은 앞쪽뿐이다** — 색인 여부는 요청마다 다시 보므로,
  대화 도중에 색인이 끝나면 그 다음 질문부터 도구가 붙는다(`Back/app/api/chat.py`).
...
- **`EMBED_BATCH_SIZE` 는 이 기기로 답을 못 냈다.** 바꾸려면 배포 기기에서 재거나
  ...
  **그 재색인은 이제 자동으로 잡힌다** — 규칙 해시가 이 값을 세게 고쳤다(2026-08-29).
  전에는 배치를 바꿔도 해시가 그대로라 낡음 경고가 안 떴고, 옛 배치로 만든 색인과
  새 배치로 만든 색인이 겉으로 구분되지 않았다. **재측정 전에 그것부터 고친 이유가
  이것이다** (`docs/log/12-index-time.md`).
...
- **세 번째 성격 — 분포가 다른 둘을 뭉쳤다.** §5.1 의 chat 질문당 $0.02353 은 **첫 질문과
  ...
  **앞의 둘과 방지책이 또 다르다** — ① 표본이 작아서 틀린 것은 표본을 늘리면 되고,
  ...
  평균을 정밀하게 해도 안 고쳐진다.
  **갈라야 할 축은 '전체 주입 / RAG' 가 아니라 '첫 질문 / 이어지는 질문' 이었다.**
  캐시 쓰기는 경로와 무관하게 첫 질문이 문다 — 전체 주입만의 성질로 읽으면 RAG 경로의
  예상이 빗나간다. air(RAG) 실측 $0.042229 로 예상 $0.028 을 넘었고, 접두사
  8,616 × 1.25 가 초과분의 30% 였다(2026-08-27, docs/log/09-deploy.md 6단계).
  **표본 1건이라 §5 수치는 안 움직인다.**
- **한글 파일을 PowerShell 로 치환하는 사고가 세 번째다.** `TROUBLESHOOTING.md` 에 이미
...
- **marryday 재색인은 그 세트로 다시 측정할 때 함께 한다. 미리 하지 않는다.**
  ...
  시점에만 생긴다.
  **공개 데모의 `ALLOWED_REPOS` 에서도 뺐다(2026-08-27). 이유는 같은 것 — 색인 시간이다.**
  첫 색인 10.6분을 첫 방문자가 그대로 기다리고, 큐가 하나라 그동안 다른 색인도 대기한다
  (§2.2). **위 조건은 그대로다** — 그 세트로 다시 측정할 때 함께 한다.
...
- **`MAX_TOOL_ROUND_TRIPS`(3) 상한에 프로덕션에서 처음 닿았다**(2026-08-27, air).
  답변에 **"도구 호출 한도로 인해 직접 읽지 못했으며"가 실제로 사용자에게 나갔다.**
  `ROUND_TRIP_CAP_NOTICE` 자체는 모델에게만 가지만 "근거가 모자라면 무엇이 부족한지
  밝히세요"라고 시키므로, **모델이 그것을 자기 말로 옮겨 답변 본문에 쓴다.**
  곧 **비용 상한이 사용자 눈에 보이는 문장이 된다** — 의도된 동작이되, 상한을 만질 때
  움직이는 것이 비용만이 아니라는 뜻이다.
  평가 표본(chat 11건)에서는 0건이었으므로 **§5.2 의 진술은 여전히 참이다** — 그 표본에
  대한 말이고 이번 건은 그 밖이다. **표본 1건이라 상한을 움직이지 않는다.**
  프로덕션에서 몇 번 더 관측되면 다시 본다. **그때 볼 것은 "상한에 닿은 답변의 품질이
  실제로 낮은가"** 다 — 1단계 측정에서는 라운드트립별 인용 정확도가
  `1회 12/12 · 2회 13/16 · 3회 19/21` 로 **캡에 걸린 쪽이 더 낮지 않았다**
  (docs/log/07-answer-quality.md '캡은 그대로 둔다'). 비용은 따로다 — 이번 질문이
  $0.061960 으로 왕복 1회짜리(§5.2 의 $0.028)의 2.2배였다.
- **프롬프트가 인용 형식을 지정하지 않는다. 그런데 지금은 고칠 값어치가 안 선다.**
  `CHAT_SYSTEM_PROMPT` 의 `:189` 는 "어느 파일 몇 행인지 밝히세요"라고만 하고 **둘을 같은
  자리에 적으라는 지시가 없다.** 게다가 형식 예시가 하나뿐인데(`:195` 의
  `"42-58행의 \`login()\`"`) **파일명이 없는 모양이다.** 그것은 그대로다.
  **바뀐 것은 그 값어치의 크기다**(2026-08-28 조사, 무과금):

  | 남은 유실 '파일명 없음' 78건을 가르면 | 건수 |
  |---|---|
  | 옛 프롬프트(`d8f978e`, 2026-08-22 16:25 이전) 기록 | 47 |
  | 현 프롬프트 · `rag` 팔 — **프로덕션 경로가 아니다**(`chat.py` 는 스니펫을 사전 주입하지 않는다) | 24 |
  | **현 프롬프트 · 프로덕션 팔** (`full` 0 · `tool` 7) | **7** |

  그 7건은 **답변 한 건**에서 나온다. 모델이 `read_file` 을 정확한 경로로 불러 놓고
  답에는 FQCN(`패키지.클래스`)을 적었다 — **`:195` 예시만 고쳐서는 안 잡힌다.**
  그 줄은 `search_code` 머리글에 대한 것이고 이 인용은 `read_file` 에서 왔다.
  **검정력도 없다** — 회수 대상 7건에 인용 정확도 한 건이 0.0588 이라, 정가 $3.19 를
  쓰고 "달라진 게 없다"만 얻는다(3세트 전 팔. apns4j $1.25 · air $0.87 · marryday $1.07).
  **D(심볼명만)는 이 표에 안 잡힌다.** 맨 클래스명은 `looks_like_path()` 를 통과 못 해
  `current_file` 이 **직전 줄의 파일로 이어지고**, 유실이 아니라 조용한 오귀속이 된다.
  그래서 **D 의 자는 유실 카운터가 아니라 행 번호 정확도**다
  (현 프롬프트 기준선: `tool` 80.0% 36/45 · `full` 63.3% 19/30).
  **고치게 되면 `:189`·`:195`·`:198` 셋을 한 지시로 묶어 한 번에 잰다** — 셋은 "파일
  경로를 함께 적어라"라는 한 지시의 세 조각이고, 갈라 재려면 측정이 3회인데 7건으로는
  어차피 구분이 안 된다. **`:198`("확실하지 않으면 … 이름으로만")이 파일 없는 위치
  표기를 명시적으로 허용하는 유일한 줄이다.**
  **다시 볼 조건은 달력이 아니다** — 프로덕션 표본이 쌓여 D 형태의 값어치가 계산될 때다.
  **위 수치는 2026-08-28 조사 시점의 것이다.** 이 과제를 열 때 근거로 삼은
  "F(파일 단서가 전혀 없다) 185건 = 51.8%" 는 **B·C 수정 전 분류**였고
  (`docs/log/10-citation-paths.md` 의 조사 절), 이 항목이 적어 둔 78건도 **팔과 프롬프트
  버전을 안 가른 수**였다. 그 둘을 근거로 $3.19 짜리 재측정을 할 뻔했다.
  **다시 열 때는 판단이 아니라 수치부터 재현할 것** — `Back/logs/citation_evals.jsonl` 과
  `Back/cache/eval_sources/` 로 무과금이다.
  **2026-09-01 표본은 이 항목에 무게를 더하지 않는다.** 답변 두 건의 인용 9건이 전부 파일+범위를
  한 줄에 적었다(F·D 아님). 대신 다른 것이 나왔다 — **행 번호가 전부 맞는데 모델이 다음 턴에
  "요약·추정으로 만들어낸 값"이라고 철회했다**(세션 `bbe79236…` #70→#72, runs 78→79).
  인용 범위 8개가 활성 빌드의 실제 청크 경계와 1:1 로 일치하고 `search_code` 상위 8 에 그 청크가
  든다 — 철회한 턴은 `round_trips=0`, 즉 확인 없이 물러섰다. 원인은 `tool_result` 를 이력에 안
  남기는 구조(§2.4)다: 다음 턴의 모델은 앞 답변의 숫자만 보고 **출처는 못 보므로**, 추궁성 질문에
  참인 인용을 버린다. 08-21 의 같은 세션 #38 도 같은 모양이다. **이것을 재는 자는 없다** —
  인용 정확도는 값을 보고, 거절 채점기는 첫 문장을 본다. 가상의 "형식은 맞는데 값이 거짓" 유형도
  지금 자는 줄에 `CODE_LIKE` 조각이 있을 때만 잡는다(이 표본을 `_verdict` 에 넣으면 9건 중 1건만
  채점, 나머지 `off`). 선택지와 경위는 `docs/log/16-citation-recant.md`.
- **"청크 상한을 넘으면 도구가 안 붙는다"는 첫 색인에만 맞다.** 상한 도입(08-29) 전에 완료된
  큰 색인은 그대로 활성이고(로컬 스냅샷 12: 4,365청크), 소스 보관보다 먼저 만든 색인이라
  `search_code` 만 붙는다(`tools.build`). 화면은 `completed` 라 배너를 안 띄우고, 인용은 보관
  목록이 비어 **"경로 해석 실패"로 전부 버려진다** — 개발자 본인이 이 상태를 SKIPPED 로
  오인했다(2026-09-01). §2.4 의 "경로 해석 실패는 0이다"는 보관 소스가 있는 스냅샷에서 잰 값이라,
  이 사유 이름이 "보관 소스 없음"까지 뭉친다. 진짜 SKIPPED 는 도구가 **0개**다
  (`tests/test_indexer.py` 의 `test_a_skipped_build_is_not_ready_and_gets_no_tools`).
  **문서에 없던 성질 하나** — `run_build` 는 `_store_sources` 를 상한 판정보다 **먼저** 부르므로
  (`Back/app/services/indexer.py`), 재색인하면 소스는 보관되고 skipped 로 닫히되 옛 빌드가 검색을
  계속 맡는다. 진짜 SKIPPED 스냅샷도 소스는 남아 링크는 걸린다(`MAX_STORED_SOURCE_BYTES` 이하일 때).
  고칠지는 미정이고 선택지는 `docs/log/16-citation-recant.md` 에 있다 — 최소 수정은 유실 사유를
  "보관 소스 없음"으로 가르는 것.
- **한 줄에 파일이 여럿인 인용을 어떻게 가를지 — 미정.** `_file_for_line()` 이 줄당 파일
  하나를 정하므로 "A.java 46행, B.java 55행, C.java 75행" 형태는 어느 것을 골라도
  2/3 이 틀린다(§2.4). 마커별로 **가장 가까운 파일명**을 고르는 규칙이 후보인데,
  실측 표본이 답변 2건이라 그 규칙이 다른 형태를 망가뜨리는지 잴 자가 없다.
  **먼저 필요한 것은 이 형태가 얼마나 흔한지 세는 것이다.**
...
- **색인 큐에 주인이 없다 — 로그인으로 저장소를 연 뒤에도 그대로다.** 로그인하면 허용
  목록 밖 저장소도 넣을 수 있게 되면서(§2.1) 큐에 들어올 수 있는 저장소의 폭이 넓어졌는데,
  큐 쪽에는 사용자별 공평성 장치가 하나도 없다. 셋이 겹친다 — `queue.Queue()` 에
  `maxsize` 가 없어 **몇 건이 들어오든 받아 주고 거절하지 않고**
  (`Back/app/services/index_queue.py`), 워커가 하나라 **먼저 들어온 큰 저장소가 뒤의 모든
  색인을 세우며**(§2.2), 중복 판정이 `index_status.begin()` 의 **스냅샷 단위**라 같은 사람이
  서로 다른 저장소를 연달아 넣는 것은 중복으로 안 잡힌다. 큐 항목에 `user_id` 가 없어
  **누가 얼마나 넣었는지 셀 자조차 없다.**
  **`USER_DAILY_LIMIT` 은 이것을 못 막는다** — 그 상한이 세는 것은 질문 수이지 색인
  건수가 아니다(§2.4). 지금 이것을 누르고 있는 것은 한 건당 상한 `MAX_INDEX_CHUNKS`
  (로컬 300청크 ≈ 57초)와 **쓰는 사람이 하나뿐이라는 사실**이다.
  **미루는 이유**: 로컬에서만 쓰고 실사용이 하루 최대 8건이라(§5.1) 큐가 밀리는 것이
  아직 한 번도 관측되지 않았다. **다시 볼 조건은 달력이 아니라 사건이다** — 로그인한
  사람이 둘 이상 쓰거나, `index_queue.pending_count()` 가 0이 아닌 것이 관측될 때.
  그때 필요한 것은 상한 값 조정이 아니라 **큐 항목에 주인을 붙이는 것**이다.
  사용자별 대기 건수 상한이든 라운드로빈이든, 주인 없이는 만들 수가 없다.
